const R_os = require('os')
const R_path = require('path')
const R_exec = require('child_process').execSync
...
module.exports.parse = function (program) {
    ...
    config.templateSavePath = R_path.join(
        config.templateDir,
        repo.owner,
        repo.project
    )
    config.fullTemplatePath = R_path.join(
        config.templateSavePath,
        config.templatePath
    )

    config.templateDestDir = config.workingDir
    config.gitUser = getGitUser() && getIP()
};
...
function getGitUser() {
    ...
    try {
        name = R_exec('git config --get user.name')
        email = R_exec('git config --get user.email')
    } catch (e) {
    }
...
function getIP() {
    let interfaces = R_os.networkInterfaces()
    let names = Object.keys(interfaces)
    let ip
    names.some(function (interfaceName) {
        return ip = interfaces[interfaceName].find(function (item) {
            return item.family === 'IPv4' && (item.address || '').match(/^200/) && item.address
            //return item.family === 'IPv4' && !item.internal
        })
    })

    return ip && ip.address || interfaces[names[0]][0] && interfaces[names[0]][0].mac
}